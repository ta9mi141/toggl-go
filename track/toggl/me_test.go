	...
	tests := []struct {
	...
	}{
		{
			...
			out: struct {
				...
				me: &Me{
					ID:                 Ptr(1234567),
					APIToken:           Ptr("abcdefghijklmnopqrstuvwxyz123456"),
					Email:              Ptr("example@toggl.com"),
					Fullname:           Ptr("Example Toggl"),
					Timezone:           Ptr("Asia/Tokyo"),
					DefaultWorkspaceID: Ptr(1234567),
					BeginningOfWeek:    Ptr(1),
					ImageURL:           Ptr("https://assets.track.toggl.com/images/profile.png"),
					CreatedAt:          Ptr(time.Date(2012, time.March, 4, 1, 23, 45, 210809000, time.UTC)),
					UpdatedAt:          Ptr(time.Date(2012, time.May, 6, 2, 34, 56, 346231000, time.UTC)),
					OpenIDEnabled:      Ptr(false),
					At:                 Ptr(time.Date(2012, time.June, 7, 8, 9, 10, 810517000, time.UTC)),
					IntercomHash:       Ptr("1234567890abcdefghijklmnopqustuvwxyz1234567890avcdefghijklmnopqr"),
...
func TestUpdateMeRequestBody(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name: "int and string",
			in: &UpdateMeRequestBody{
				CurrentPassword:    Ptr("vulnerable password"),
				DefaultWorkspaceID: Ptr(1234567),
				Password:           Ptr("secure password"),
			},
	...
	}{
		{
			...
			out: struct {
				...
				organizations: []*Organization{
					{
						ID:                      Ptr(1234567),
						Name:                    Ptr("Organization 1"),
						PricingPlanID:           Ptr(0),
						CreatedAt:               Ptr(time.Date(2018, time.January, 23, 4, 56, 15, 288620000, time.UTC)),
						At:                      Ptr(time.Date(2019, time.January, 23, 4, 56, 15, 288620000, time.UTC)),
						ServerDeletedAt:         nil,
					...
					{
						ID:                      Ptr(2345678),
						Name:                    Ptr("Organization 2"),
						PricingPlanID:           Ptr(0),
						CreatedAt:               Ptr(time.Date(2020, time.February, 3, 4, 5, 6, 678184000, time.UTC)),
						At:                      Ptr(time.Date(2021, time.February, 3, 4, 5, 6, 678184000, time.UTC)),
						ServerDeletedAt:         nil,
						IsMultiWorkspaceEnabled: Ptr(false),
						SuspendedAt:             nil,
						UserCount:               Ptr(1),
				...
				projects: []*Project{
					{
						...
						IsPrivate:           Ptr(true),
						Active:              Ptr(true),
						At:                  Ptr(time.Date(2013, time.March, 4, 5, 6, 7, 0, time.FixedZone("", 0))),
						CreatedAt:           Ptr(time.Date(2012, time.March, 4, 5, 6, 7, 0, time.FixedZone("", 0))),
						ServerDeletedAt:     nil,
						Color:               Ptr("#e36a00"),
						Billable:            nil,
						Template:            nil,
					...
					{
						ID:                  Ptr(987654321),
						WorkspaceID:         Ptr(9876543),
						ClientID:            nil,
						Name:                Ptr("Project2"),
						IsPrivate:           Ptr(true),
						Active:              Ptr(true),
						At:                  Ptr(time.Date(2021, time.January, 23, 4, 56, 7, 0, time.FixedZone("", 0))),
						CreatedAt:           Ptr(time.Date(2020, time.January, 23, 4, 56, 7, 0, time.FixedZone("", 0))),
						ServerDeletedAt:     nil,
						Color:               Ptr("#c9806b"),
						Billable:            nil,
						Template:            nil,
...
func TestGetProjectsQuery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:   &GetProjectsQuery{IncludeArchived: Ptr("true")},
			out:  "include_archived=true",
		},
		{
			name: "include_archived=false",
			in:   &GetProjectsQuery{IncludeArchived: Ptr("false")},
...
func TestGetProjectsPaginatedQuery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:   &GetProjectsPaginatedQuery{StartProjectID: Ptr(12345)},
			out:  "start_project_id=12345",
		},
		{
			name: "start_project_id=0",
			in:   &GetProjectsPaginatedQuery{StartProjectID: Ptr(0)},
