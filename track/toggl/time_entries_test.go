	...
	}{
		{
			...
			out: struct {
				...
				timeEntries: []*TimeEntry{
					{
						ID:              Ptr(1234567890),
						WorkspaceID:     Ptr(1234567),
						ProjectID:       Ptr(123456789),
						TaskID:          nil,
						Billable:        Ptr(false),
						Start:           Ptr(time.Date(2012, time.March, 4, 5, 6, 20, 0, time.FixedZone("", 0))),
						Stop:            Ptr(time.Date(2012, time.March, 4, 5, 6, 23, 0, time.UTC)),
						Duration:        Ptr(3),
						Description:     Ptr("test time entry"),
						Tags:            []*string{Ptr("billed")},
						TagIDs:          []*int{Ptr(1234567)},
						Duronly:         Ptr(false),
						At:              Ptr(time.Date(2022, time.March, 4, 5, 6, 7, 0, time.FixedZone("", 0))),
						ServerDeletedAt: nil,
						UserID:          Ptr(9876543),
						UID:             Ptr(9876543),
						WID:             Ptr(1234567),
					...
					},
					{
						ID:              Ptr(2345678901),
						WorkspaceID:     Ptr(1234567),
						ProjectID:       Ptr(234567890),
						TaskID:          nil,
						Billable:        Ptr(false),
						Start:           Ptr(time.Date(2022, time.January, 2, 3, 47, 41, 0, time.FixedZone("", 0))),
						Stop:            Ptr(time.Date(2022, time.January, 2, 3, 48, 1, 0, time.UTC)),
						Duration:        Ptr(20),
						Description:     Ptr("test time entry"),
						Tags:            []*string{Ptr("billed"), Ptr("toggl-go")},
						TagIDs:          []*int{Ptr(1234567), Ptr(2345678)},
						Duronly:         Ptr(false),
						At:              Ptr(time.Date(2022, time.March, 4, 5, 6, 7, 0, time.FixedZone("", 0))),
						ServerDeletedAt: nil,
						UserID:          Ptr(9876543),
...
func TestGetTimeEntriesQuery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			in:   &GetTimeEntriesQuery{Before: Ptr("2022-07-01")},
			out:  "before=2022-07-01",
		},
		{
			name: "since=1656687597",
			in:   &GetTimeEntriesQuery{Since: Ptr(1656687597)},
			out:  "since=1656687597",
		},
		{
			name: "end_date=2022-07-07&start_date=2022-07-01",
			in: &GetTimeEntriesQuery{
				StartDate: Ptr("2022-07-01"),
				EndDate:   Ptr("2022-07-07"),
			},
	...
	}{
		{
			...
			out: struct {
				...
				timeEntry: &TimeEntry{
					...
					TaskID:          nil,
					Billable:        Ptr(false),
					Start:           Ptr(time.Date(2020, time.January, 23, 4, 56, 31, 0, time.FixedZone("", 0))),
					Stop:            nil,
					Duration:        Ptr(-1579722991),
					Description:     Ptr("running time entry"),
					Tags:            []*string{Ptr("toggl-go")},
					TagIDs:          []*int{Ptr(1234567)},
					Duronly:         Ptr(false),
					At:              Ptr(time.Date(2020, time.January, 23, 4, 56, 34, 0, time.FixedZone("", 0))),
					ServerDeletedAt: nil,
					UserID:          Ptr(1234567),
					UID:             Ptr(1234567),
					WID:             Ptr(1234567),
	...
	}{
		{
			...
			out: struct {
				...
				timeEntry: &TimeEntry{
					...
					TaskID:          nil,
					Billable:        Ptr(false),
					Start:           Ptr(time.Date(2021, time.July, 6, 5, 4, 3, 0, time.UTC)),
					Stop:            Ptr(time.Date(2021, time.July, 6, 5, 9, 3, 0, time.UTC)),
					Duration:        Ptr(300),
					Description:     Ptr("created manually"),
					Tags:            nil,
					TagIDs:          nil,
					Duronly:         Ptr(false),
					At:              Ptr(time.Date(2022, time.July, 6, 5, 4, 3, 0, time.FixedZone("", 0))),
					ServerDeletedAt: nil,
					UserID:          Ptr(1234567),
					UID:             Ptr(1234567),
					WID:             Ptr(1234567),
...
func TestCreateTimeEntryRequestBody(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			in: &CreateTimeEntryRequestBody{
				WorkspaceID: Ptr(1234567),
				Start:       Ptr(time.Date(2022, time.July, 6, 5, 4, 3, 0, time.UTC)),
				Duration:    Ptr(300),
				CreatedWith: Ptr("toggl-go"),
				Description: Ptr("created manually"),
				ProjectID:   Ptr(123456789),
			},
		...
		},
		{
			...
			in: &CreateTimeEntryRequestBody{
				...
				Description: Ptr("created manually"),
				ProjectID:   Ptr(123456789),
				TagIDs:      []*int{Ptr(1234567), Ptr(9876543)},
			},
	...
	tests := []struct {
	...
	}{
		{
			...
			out: struct {
				...
				timeEntry: &TimeEntry{
					ID:              Ptr(1234567890),
					WorkspaceID:     Ptr(1324567),
					ProjectID:       Ptr(234567890),
					TaskID:          nil,
					Billable:        Ptr(false),
					Start:           Ptr(time.Date(2022, time.July, 6, 5, 43, 31, 0, time.UTC)),
					Stop:            Ptr(time.Date(2022, time.July, 6, 5, 44, 37, 0, time.UTC)),
					Duration:        Ptr(66),
					Description:     Ptr("updated time entry"),
					Tags:            []*string{Ptr("toggl-go")},
					TagIDs:          []*int{Ptr(3456789)},
					Duronly:         Ptr(false),
					At:              Ptr(time.Date(2022, time.July, 7, 12, 34, 56, 0, time.FixedZone("", 0))),
					ServerDeletedAt: nil,
					UserID:          Ptr(1234567),
					UID:             Ptr(1234567),
					WID:             Ptr(1324567),
